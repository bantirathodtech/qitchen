...
class ProductViewModel extends ChangeNotifier {
  ...
  ProductViewModel({
    ...
    required ProductsNotifier state,
        ...
        _state = state {
    _state.addListener(_onStateChanged); // Step 1: Listen to state changes
  }

  // Step 2: Forward state getters
  ...
  Map<String, List<ProductModel>> get productsByCategory => _state.productsByCategory;
  Map<String, List<ProductModel>> get productsByMenu => _state.productsByMenu;

  // Step 3: Fetch products and update state
  Future<void> fetchProducts(String storeId, {bool forceRefresh = false}) async {
    try {
      Future.microtask(() => _state.setLoading(true)); // Set loading state
      Future.microtask(() => _state.setError('')); // Clear previous errors
      final products = await _repository.getProducts(storeId, forceRefresh: forceRefresh);
      Future.microtask(() => _state.updateProducts(products)); // Update products
    } catch (e) {
      Future.microtask(() => _state.setError('Failed to fetch products: $e')); // Set error
    } finally {
      Future.microtask(() => _state.setLoading(false)); // Reset loading
    }
  }
  ...
  void setSelectedCategory(String category) => _state.setSelectedCategory(category);
  ...
  List<ProductModel> searchProducts(String query) => _state.searchProducts(query);
  ...
  @override
  void dispose() {
    _state.removeListener(_onStateChanged); // Step 7: Clean up listener
    super.dispose();
  }
}
...
//   //   }
//   // }
//   // In product_viewmodel.dart
//   Future<void> fetchProducts(String storeId, {bool forceRefresh = false}) async {
//     try {
//       // Set loading state
//       Future.microtask(() => _state.setLoading(true));
//
//       // Clear any previous errors
//       Future.microtask(() => _state.setError(''));
//
//       // Fetch the products
//       final products = await _repository.getProducts(storeId, forceRefresh: forceRefresh);
//
//       // Update state with products after build
//       Future.microtask(() => _state.updateProducts(products));
//     } catch (e) {
//       // Set error state after build
//       Future.microtask(() => _state.setError('Failed to fetch products: $e'));
//     } finally {
//       // Set loading state to false after build
//       Future.microtask(() => _state.setLoading(false));
//     }
//   }
//
//   // Filter Methods
