...
class DynamicTabsController extends ChangeNotifier {
  ...
  DynamicTabsController({required this.vsync, required List<String> initialTabs})
      : _tabs = initialTabs {
    // Step 1: Initialize TabController with initial tabs
  ...
  // Step 2: Update tabs dynamically
  void updateTabs(List<String> newTabs) {
    if (newTabs.length != _tabs.length) {
      _tabController.dispose();
      _tabController = TabController(length: newTabs.length, vsync: vsync);
      _tabs = newTabs;
      notifyListeners();
    }
  }
...
class DynamicTabs extends StatefulWidget {
  final List<String> tabs; // List of category names from ViewModel
  final Widget Function(BuildContext, String) tabBuilder; // Builder for tab content
...
// class _DynamicTabsState extends State<DynamicTabs> with SingleTickerProviderStateMixin {
//   late DynamicTabsController _tabsController;
class _DynamicTabsState extends State<DynamicTabs> with TickerProviderStateMixin {
  late DynamicTabsController _tabsController;

  @override
  void initState() {
    super.initState();
    // Step 3: Initialize controller with tabs
    _tabsController = DynamicTabsController(vsync: this, initialTabs: widget.tabs);
  }

  @override
  void didUpdateWidget(DynamicTabs oldWidget) {
    super.didUpdateWidget(oldWidget);
    // Step 4: Update tabs if they change
  ...
  Widget build(BuildContext context) {
    // Step 5: Build tab bar and tab view
    return Column(
      children: [
        SizedBox(
          ...
          child: TabBar(
            ...
            tabs: List.generate(
              widget.tabs.length,
                  (index) => widget.customTabBuilder?.call(context, index, widget.tabs[index]) ?? Tab(text: widget.tabs[index]),
            ),
          ),
        ),
        Expanded(
          child: TabBarView(
            controller: _tabsController.tabController,
            children: widget.tabs.map((tab) => widget.tabBuilder(context, tab)).toList(),
          ),
        ),
      ],
    );
