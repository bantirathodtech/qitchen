...
class KitchenOrderRepository {
  ...
  Future<KitchenOrderResponse> createRedisOrder({
    ...
    try {
      ...
      // Create order matching Redis API structure exactly
      final order = KitchenOrder(
        ...
        status: status.toLowerCase(), // Ensure status is lowercase as per API
        line: lineItems
            .map((item) => KitchenOrderItem(
                  ...
                  status:
                      item.status.toLowerCase(), // Ensure status is lowercase
      ...
      AppLogger.logInfo(
          '$TAG Kitchen order created successfully: ${response.message}');
      return response;
    } catch (e) {
