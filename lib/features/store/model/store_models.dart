...
class RestaurantModel {
  ...
  factory RestaurantModel.fromJson(Map<String, dynamic> json) {
    ...
    return RestaurantModel(
      // Use store name if available, otherwise provide a default
      name: json['name']?.trim() ?? 'Unnamed Restaurant',
      // Default description when null
      shortDescription: json['shortDescription'] ?? 'No description available to display to the user, this is sample 2 line description',
      // Default image placeholder when null or empty
      storeImage: json['storeImage']?.trim() ?? 'https://via.placeholder.com/150?text=No+Image',
      // Ensure storeId is never null
      storeId: json['storeId'] ?? 'NO_ID',
      // Pass empty map if csBunit is null
      businessUnit: BusinessUnitModel.fromJson(json['csBunit'] ?? {}),
      // Empty list if timings are null
      timings: (json['storeTimings'] as List? ?? [])
          .map((e) => TimingModel.fromJson(e))
          .toList(),
      // Empty list if holidays are null
      holidays: (json['storeHolidays'] as List? ?? [])
          .map((e) => HolidayModel.fromJson(e))
          .toList(),
    );
