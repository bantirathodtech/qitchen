...
class _ConfirmationDisplayScreenState extends State<ConfirmationDisplayScreen> {
  ...
  //     });
  //
  //     AppLogger.logInfo('$TAG Processing order_shared_common: ${widget.order_shared_common.documentno}');
  //     AppLogger.logDebug('$TAG Order data: ${widget.order_shared_common.toJson()}');
  //     AppLogger.logDebug('$TAG Line items count: ${widget.order_shared_common.line.length}');
  //
  //     // Create a deep clone of the order_shared_common with line items
  //     final processedOrder = _cloneOrder(widget.order_shared_common);
  //
  //     // Validate order_shared_common data
  //     _validateOrder(processedOrder);
  //
  //     // Create payment order_shared_common first
  ...
  //
  //     if (!paymentSuccess) {
  //       throw Exception('Failed to create payment order_shared_common');
  //     }
  //
  //     AppLogger.logInfo('$TAG Payment order_shared_common created successfully');
  //
  ...
  //
  //       AppLogger.logDebug(
  //           '$TAG Creating kitchen order_shared_common for center: $center with ${items.length} items');
  //
  //       final kitchenOrderLines = items
  //           .map((item) => KitchenOrderItem(
  //         mProductId: item.mProductId,
  //         name: item.name ?? 'Unknown Item',
  //         qty: item.qty, // From PaymentOrderItem
  //         notes: '',
  ...
  //
  //       if (!kitchenSuccess) {
  //         throw Exception('Failed to create kitchen order_shared_common for center: $center');
  //       }
  //
  //       AppLogger.logInfo('$TAG Kitchen order_shared_common created for center: $center');
  //     }
  //
  ...
  Future<void> _processOrder() async {
    try {
      ...
      // Simply log the order_shared_common details for debugging
      AppLogger.logInfo('$TAG Displaying order_shared_common: ${widget.order.documentno}');
      AppLogger.logDebug('$TAG Order data: ${widget.order.toJson()}');

      // No order_shared_common creation logic here!
      // Just set the order_shared_common in the state so UI can display it
      _paymentViewModel.setDisplayOrder(widget.order);
      _kitchenViewModel.setDisplayOrder(widget.order);


      setState(() => _isProcessing = false);
    } catch (e, stackTrace) {
      AppLogger.logError('$TAG Error displaying order_shared_common: $e');
      AppLogger.logError('$TAG Stack trace: $stackTrace');
      setState(() {
        _error = e.toString();
        _isProcessing = false;
...
// Helper Methods
  ...
  void _validateOrder(Order order) {
    if (order.documentno.isEmpty) {
      throw Exception('Invalid order_shared_common: Document number is missing');
    }
  ...
  Map<String, List<PaymentOrderItem>> _groupItemsByCenter(List<PaymentOrderItem> items) {
