...
// updated on 17/03/25
  double _parseBalance(dynamic value) {
    ...
    if (value is String) {
      try {
        ...
        // If string parsing fails, log and return 0.0
        AppLogger.logError('[WalletService] Error parsing balance: $e');
        return 0.0;
      }
    }
  ...
  Future<Map<String, dynamic>> getWalletBalance(String customerId) async {
    try {
      AppLogger.logInfo('[WalletService] Fetching wallet balance for customer: $customerId');


      final storedWalletId = await AppPreference.getWalletId();
      final effectiveWalletId = storedWalletId ?? customerId;

      AppLogger.logInfo('[WalletService] Using effectiveWalletId: $effectiveWalletId');
      ...
      AppLogger.logInfo('[WalletService] Raw wallet balance response: $response');
      if (response != null) {
        // Log the type of balance_Amt
        AppLogger.logInfo('[WalletService] balance_Amt type: ${response['balance_Amt']?.runtimeType}');
        AppLogger.logInfo('[WalletService] balance_Amt value: ${response['balance_Amt']}');
