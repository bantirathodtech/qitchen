...
class CartManager with ChangeNotifier {
  ...
  String? get currentRestaurant =>
      _cartItemsByRestaurant.isEmpty ? null : _cartItemsByRestaurant.keys.first;
  List<String> get restaurants => _cartItemsByRestaurant.keys.toList();

  // Remove _firstRestaurantCsBunitId and use storeProvider directly
          ...
          return MapEntry(
            ...
            (value as List<dynamic>)
                .map((item) => CartItem(
                      ...
                      selectedAddons:
                          (item['selectedAddons'] as Map<String, dynamic>).map(
                        (key, value) => MapEntry(
                          key,
                          (value as List)
  ...
  void updateItemNotes(String restaurantName, CartItem cartItem, String notes) {
    ...
    if (items != null) {
      final index = items.indexWhere((item) =>
          ...
          item.selectedAddons == cartItem.selectedAddons);
...
// Save cart data to storage
  ...
  void addToCart(
    ...
    try {
      AppLogger.logInfo('$TAG: Adding to cart - Product: ${product.name}, '
      ...
      AppLogger.logDebug(
          '$TAG: Restaurant validation result: ${restaurantValidation.isValid}');
      if (!restaurantValidation.isValid) {
        _showError(context,
            restaurantValidation.message ?? 'Invalid restaurant selection');
        return;
      }
      ...
      if (product.addOnGroups?.isNotEmpty ?? false) {
        ...
        AppLogger.logDebug(
            '$TAG: Addon validation result: ${addonValidation.isValid}');
        if (!addonValidation.isValid) {
          _showError(
              context, addonValidation.message ?? 'Invalid addon selection');
          return;
        }
      }
  ...
  // Find existing cart item for a product in a specific restaurant
  ...
  // Update an existing cart item in a specific restaurant
  void _updateCartItem(String restaurantName, CartItem updatedItem) {
    final items = _cartItemsByRestaurant[restaurantName];
    if (items == null) return;

    final index = items.indexWhere(
      (item) => item.product.productId == updatedItem.product.productId,
    );
  ...
  // Add a new cart item to a specific restaurant
  ...
  CartValidation validateRestaurant(String restaurantName) {
    // if (currentRestaurant != null && currentRestaurant != restaurantName) {
  ...
  CartValidation validateAddons(
      ProductModel product, Map<String, List<AddOnItem>> addons) {
    try {
      for (var group in product.addOnGroups ?? []) {
        if (group.name == "Customes") continue;

        final selectedAddons = addons[group.id] ?? [];
        if (selectedAddons.length < group.minqty ||
            selectedAddons.length > group.maxqty) {
  ...
  void incrementCartItem(String restaurantName, CartItem cartItem) {
    ...
    if (index != -1) {
      items[index] = cartItem.copyWith(
        ...
        totalPrice:
            cartItem.totalPrice * ((cartItem.quantity + 1) / cartItem.quantity),
      );
      _saveCartData();
      notifyListeners();
    }
...
// Clear specific restaurant's cart
...
// Updated numeric methods to use num consistently
  num getTotalPrice(String restaurantName) {
    return _cartItemsByRestaurant[restaurantName]?.fold<num>(
          0,
...
// Get cart subtotal
  num get subtotal {
    return _cartItemsByRestaurant.values
        .expand((items) => items)
        .fold<num>(0, (sum, item) => sum + item.totalPrice);
  }
...
// Get cart total with taxes
  num get total {
    final taxes =
        getTotalTaxesByCategory().values.fold<num>(0, (sum, tax) => sum + tax);
